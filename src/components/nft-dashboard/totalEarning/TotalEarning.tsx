...
export const TotalEarning: React.FC = () => {
  ...
  const { totalEarningData, days } = useMemo(() => {
    const earningData = {
      data: rates.map((item: Earning) => Number(item.usd_value)),
    };
  ...
  const latestRate = rates.length > 0 ? Number(rates[rates.length - 1]?.usd_value) : 0;
  const previousRate = rates.length > 1 ? Number(rates[rates.length - 2]?.usd_value) : 0;
  const isIncreased = !isNaN(latestRate) && !isNaN(previousRate) ? latestRate > previousRate : false;
  const rateDifference = !isNaN(latestRate) && !isNaN(previousRate) ? ((latestRate - previousRate) / previousRate) * 100 : 0;
  ...
  const formattedLatestRate = !isNaN(latestRate) ? parseFloat(latestRate.toFixed(0)) : 0;
...
//   const { totalEarningData, days } = useMemo(() => {
//     const earningData = {
//       data: bitcoinRates.map((item) => Number(item.usd_value)),  // Ensure values are numbers
//     };
...
//   const latestRate = bitcoinRates.length > 0 ? Number(bitcoinRates[bitcoinRates.length - 1]?.usd_value) : 0;  // Convert to number
//   const previousRate = bitcoinRates.length > 1 ? Number(bitcoinRates[bitcoinRates.length - 2]?.usd_value) : 0;  // Convert to number
//   const isIncreased = !isNaN(latestRate) && !isNaN(previousRate) ? latestRate > previousRate : false;
//   const rateDifference = !isNaN(latestRate) && !isNaN(previousRate) ? ((latestRate - previousRate) / previousRate) * 100 : 0;
